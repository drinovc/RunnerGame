...
var goals = [
	{ name: 'Razor', distance: 2601 },
	{ name: 'Jošt', distance: 845 },
	{ name: 'Kriška gora', distance: 1471 },
	{ name: 'Triglav', distance: 2864 },
	{ name: 'Kališče', distance: 1534 },
	{ name: 'Stol', distance: 1673 }
];
...
function generateRandomObject() {
	...
	var obj = null;
	switch(random(0,3)) {
		case 0: obj = o.coin; break;
		case 1: obj = o.rock; break;
		case 2: obj = o.tree; break;
		case 3: obj = o.coffee; break;
	...
	if(!paused) {
		...
		if(distance >= goalDistance - gridBuffer - gridH) {
			...
			if(gameGrid.length < gridH + gridBuffer) {
				switch(random(0,3)) {
					case 0: generateCoins(); break;
					case 1: generateRocks(); break;
					case 2: generateTrees(); break;
					case 3: generateRandomObject(); break;
					default: console.log('Not enough generators');
				}
			}
		}
	}
...
function draw() {
	...
	if(lives === 0 || win) {
		...
		if(win) {
			...
			ctx.fillText('REKORD: ' + bestScore, w/2, h/2 + unit/2);
		}
		else {
			ctx.font = fontFactor*unit/1.5 + 'px ' + font;
			ctx.fillText('KONEC IGRE', w/2, h/2 + unit/20);
			ctx.font = fontFactor*unit/2 + 'px ' + font;
			ctx.fillText('REKORD: ' + bestScore, w/2, h/2 + unit/2);
		}
	...
	ctx.textAlign = 'left';
	ctx.font = fontFactor*unit/2 + 'px ' + font;
	ctx.fillText('DO VRHA: ' + Math.max(0, goalDistance - distance) + 'm', unit/8, unit/2);
	ctx.fillText('TOCKE:   ' + score , unit/8, unit/1.25);
	ctx.closePath();
...
function start() {
	if(--counter === 0) {
		showMsg(
			'<h2>Katja pleza</h2>' +
			'<br>' +
			'Katja zelo rada pleza. Zaradi slabega vremena ali zaradi premalo časa pa ne more vsak dan v hribe. '  +
			'Vsaj ne v realnosti. Lahko pa vsak dan spleza na hrib tukaj.' +
			'<br>Pomagaj ji priplezati do vrha.' +
			'<br>' +
			'<br>Pritisni levo polovico zaslona za premik levo in desno za desno.' +
			'<br>Jej pico za točke in pij kavo za energijo in moč, da premagaš ovire.' +
			'<br>' +
			'<br><br><b>Pritisni za plezanje: <br>' + goalName + '(' + goalDistance + 'm)</b>'
		);
		initialized = true;
	}
}
